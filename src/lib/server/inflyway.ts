...
async function inflywayRequest<T extends InflywayApiResponse>(
  path: string,
...
export async function createInflywayCheckout(payload: InflywayCheckoutPayload) {
  const data = await inflywayRequest<InflywayCheckoutResponse>(
    "/checkout/create",
  ...
  if (!data.orderId || !data.orderUrl) {
    throw new InflywayRequestError(
      "Inflyway did not return a payment link",
      502,
      data
    )
  }
...
export function isInflywayOrderPaid(
  ...
  const paymentSuccessTime =
    order.paymentSuccessTime ?? order.paySuccessTime ?? order.paymentTime ?? null

  if (paymentSuccessTime) return true
  if (["10", "20", "SUCCESS", "PAID", "COMPLETED", "01"].includes(paymentStatus))
    return true
  if (["PAID", "SUCCESS", "CONFIRMED", "COMPLETED", "POCS"].includes(orderStatus))
    return true
