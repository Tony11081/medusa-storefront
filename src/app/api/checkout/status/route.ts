...
import { shouldUseSecureCookies } from "@lib/data/cookies"
import {
  InflywayRequestError,
  getInflywayOrder,
  isInflywayOrderPaid,
} from "@lib/server/inflyway"
import { finalizeMedusaCart } from "@lib/server/medusa-checkout"
...
function isLookupMiss(error: unknown) {
  ...
  const bodyError =
    typeof error.body === "object" && error.body !== null && "error" in error.body
      ? normalize((error.body as any).error)
      : ""
...
export async function GET(request: NextRequest) {
  ...
  const countryCode =
    normalize(request.nextUrl.searchParams.get("countryCode")).toLowerCase() ||
    "gb"
  const query = orderId || orderRef

  if (!query) {
    return NextResponse.json(
      { message: "orderId or orderRef is required" },
      { status: 400 }
    )
  }
  ...
  try {
        ...
        medusaSync = {
          ...
          error:
            error instanceof Error
              ? error.message
              : "Could not sync paid order to Medusa",
        }
      }
    }
    ...
    const response = NextResponse.json({
      ...
      cartId: cartId || null,
      confirmedUrl:
        paid && medusaSync?.orderId
          ? `/${countryCode}/order/${medusaSync.orderId}/confirmed`
          : null,
      medusaSync,
      paid,
